...
class IncidentHandler(BaseHandler):
    ...
    def read(self,
        ...
        if incident_id:
            ...
            if filter_time:
                return_objs = Incident.objects\
                    .filter(created__gte=filter_time)\
                    .filter(validated=True)\
                    .order_by(ORDER_FIELD_VALUES[order_field])\
                    .reverse()
            else:
                return_objs = Incident.objects\
                    .filter(validated=True)\
            ...
            if scope == "current":
                return_objs = [incident \
                    for incident in return_objs \
                    if not incident.ended_count() > 3]
        ...
        if request.content_type:
            try:
                ...
                if 'line_id' in data:
                    ...
                    line, is_created = Line.objects\
                        .get_or_create(name=data['line_name'].strip())
                if not line:
                    logger.error('Incident CRUD Handler got request with no line',
